...
RSpec.describe StudentCustomAttribute do
  let(:student) do
    Student.create(name: 'Test Student', uin: '123456789', grade_ryg: 'G', gender: 'Male', ethnicity: 'Asian',
                   nationality: 'American', expected_graduation: '2025-05-15', university_classification: 'Junior', status: 'Active', sexual_orientation: 'Heterosexual', date_of_birth: '2000-01-01', email: 'test@example.com')
  end
  ...
  describe 'validations' do
    it 'is valid with a student, custom attribute, and value' do
      student_custom_attribute = described_class.new(student:, custom_attribute:,
                                                     value: 'Test Value')
      expect(student_custom_attribute).to be_valid
    end

    it 'is invalid without a student' do
      student_custom_attribute = described_class.new(custom_attribute:, value: 'Test Value')
    ...
    it 'is invalid without a custom attribute' do
      student_custom_attribute = described_class.new(student:, value: 'Test Value')
      expect(student_custom_attribute).not_to be_valid
